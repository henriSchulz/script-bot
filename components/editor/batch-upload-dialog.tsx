  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{dict.blockEditor.batchUpload.title}</DialogTitle>
          <DialogDescription>
            {dict.blockEditor.batchUpload.description}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="grid gap-1.5 flex-1">
                        ...
                        fileUrl ? (
                            <a href={`${fileUrl}#page=${page}`} target="_blank" rel="noopener noreferrer">
                                <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold text-muted-foreground bg-background hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors">
                                    {dict.blockEditor.batchUpload.page.replace("{page}", page.toString())}
                                </span>
                            </a>
                        ) : (
                  ...
                  <p className="text-sm text-muted-foreground">
                    {dict.blockEditor.batchUpload.uploadPrompt}
                  </p>
                </div>
              </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={uploading}>
            {dict.blockEditor.batchUpload.cancel}
          </Button>
          <Button onClick={handleUpload} disabled={uploading || Object.keys(files).length === 0} className="gap-2">
            {uploading ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" />
                {dict.blockEditor.batchUpload.uploading} ({Math.round(progress)}%)
              </>
            ) : (
              <>
                <Upload className="h-4 w-4" />
                {dict.blockEditor.batchUpload.uploadSelected.replace("{count}", Object.keys(files).length.toString())}
              </>
            )}
          </Button>
        </DialogFooter>
